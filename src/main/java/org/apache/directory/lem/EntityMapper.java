...
class EntityMapper
{
	...
	static void unloadMap(Entity inmodel, Entity inentity, MultiValuedMap map) throws LemException
	{
	...
	/**
	 * This routine uses Java Reflection to iterate over an object's fields and
	 * convert into LDAP attributes. It uses two objects: the entity's name is
	 * pulled from "model", the values from the "entity".
	 *
	 * @param inmodel The mappings between the logical and physical data model.
	 * @param inentity The data to be loaded into the directory. It's formatted
	 * using the logical structure.
	 * @param isRead If true the caller is performing a read or search,
	 * otherwise it's an edit.
	 * @throws LemException
	 */
	static MultiValuedMap loadMap(Entity inmodel, Entity inentity, boolean isRead) throws LemException
	{
		Field[] fields = inentity.getClass().getDeclaredFields();
		...
		try
		{
			for (Field entity : fields)
			{
				...
				switch (entity.getType().getSimpleName())
				{
					...
					{
						LOG.debug("LIST: {}", entity.get(inmodel));
						if ((!isRead && model.get(inmodel) != null && entity.get(inentity) != null)
							|| (isRead && model.get(inmodel) != null))
						{
							List modelAttrs = (List) model.get(inmodel);
							...
							for (var entityAttrValue : (List) field.get(object))
							{
								String modelAttrName;
								if (modelAttrs.size() > 1)
								{
