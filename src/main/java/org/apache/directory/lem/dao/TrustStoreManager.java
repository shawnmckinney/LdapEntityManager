...
/**
 * Implement the X509TrustManager interface which will be used during JSSE
 * truststore manager initialization for LDAP client-to-server communications
 * over TLS/SSL. It is used during certificate validation operations within
 * JSSE.
 * <p>
 * There are the controlling fortress.properties:
 * <ul>
 * <li>trust.store : contains the name of the truststore (must be fully
 * qualified iff trust.store.onclasspath=false</li>
 * <li>trust.store.password : contains the pw for the specified truststore</li>
 * <li>trust.onclasspath : if false name must be fully qualified, otherwise file
 * must be on classpath as named</li>
 * </ul>
 *
 * Note: This class allows self-signed certificates to pass the validation
 * checks, if its root certificate is found in the truststore.
 *
 * @author <a href="mailto:dev@directory.apache.org">Apache Directory
 * Project</a>
 */
public final class TrustStoreManager implements X509TrustManager, Serializable
{
	...
	// Logging
	private static final String CLS_NM = TrustStoreManager.class.getName();
	...
	/**
	 * Constructor used by connection configuration utility to load trust store
	 * manager.
	 *
	 ...
	 * @param isExamineValidity boolean var determines if certificate will be
	 * examined for valid dates on load.
	 */
	public TrustStoreManager(final String trustStoreFile, final char[] trustStorePw,
		final String trustStoreFormat, final boolean isExamineValidity)
	{
		if (trustStoreFile == null)
		{
			// Cannot continue, throw an unchecked exception:
			throw new RuntimeException("TrustStoreManager constructor : input file name is null");
		}
	...
	/**
	 * Return array of trust managers to caller. Will verify that current date
	 * is within certs validity period.
	 *
	...
	private synchronized X509TrustManager[] getTrustManagers(final X509Certificate[] x509Chain) throws
		CertificateException
	{
		if (Config.getBoolean(Ids.TRUST_STORE_ON_CLASSPATH, true) == false)
		{
		...
		CertificateException
	{
		...
		{
			throw new CertificateException("TrustStoreManager.getTrustManagers : file not found");
		}
		try
		{
		...
		} catch (IOException e)
		{
			...
			LOG.warn("TrustStoreManager.getTrustManagers on input stream close "
	 ...
	 * @throws CertificateException if problem occurs during TrustManager
	 * initialization.
	 */
	private X509TrustManager[] loadTrustManagers(final KeyStore trustStore) throws CertificateException
	{
		...
		{
			throw new CertificateException("TrustStoreManager.loadTrustManagers caught "
				+ "NoSuchAlgorithmException", e);
		} catch (KeyStoreException e)
		{
			throw new CertificateException("TrustStoreManager.loadTrustManagers caught KeyStoreException", e);
		}
	 ...
	 *
	 * @return instance of JSSE KeyStore containing the LDAP Client's TrustStore
	 * file info.
	 *
	...
	private KeyStore getTrustStore() throws CertificateException
	{
		...
		{
			throw new CertificateException("TrustStoreManager.getTrustManagers caught KeyStoreException", e);
		}
		...
		{
			throw new CertificateException("TrustStoreManager.getTrustManagers caught "
				+ "NoSuchAlgorithmException", e);
		} catch (IOException e)
		{
			throw new CertificateException("TrustStoreManager.getTrustManagers caught KeyStoreException", e);
		} finally
		{
			...
			if (trustStoreInputStream != null)
			{
				...
				{
					// Eat this ioexception because it shouldn't be a problem, but log just in case:
					LOG.warn("TrustStoreManager.getTrustStore finally block on input stream close "
						+ "operation caught IOException={}", e.getMessage());
				}
			}
		}
	...
	private InputStream getTrustStoreInputStream() throws CertificateException
	{
		...
		if (null == result)
		{
			throw new CertificateException("TrustStoreManager.getTrustStoreInputStream file does not exist on fortress classpath");
		}
		return result;
	}
}
